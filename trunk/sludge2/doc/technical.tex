...
\[
	\hat{C}_Q = \dfrac{2}{|AB|} \dfrac{\det(\myvec{AQ},\myvec{QB})}{|AQ||AB|}
\]
...
The geometrical excess curvatures at the arc extremity are
\[
	\kappa_A = C_A - \hat{C}_A \text{ and } \kappa_B = C_B - \hat{C}_B
\]
so that we can defined the first order authorized excess curvature
\[
	\kappa(\mu) = \kappa_A + \mu (\kappa_B - \kappa_A)
\]
with
\[
...
Accordingly,
\[
	\delta_\kappa = \kappa_Q - \kappa(\mu)
\]
is the true curvature excess for $Q$.
