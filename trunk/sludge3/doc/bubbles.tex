...
$$
	\sigma^E_i = \dfrac{P_\phi - P_{i-1}}{L_i}.
$$
...
$$
Using $h_i = \dfrac{L_i}{2}$, the pressure at the middle of the segment can be expressed as
$$
	P_{i,m}^E = \dfrac{1}{\delta_x} \left\lbrack
	h_i P_i + \left(\delta_x - h_i\right) P_{i-1}
	\right\rbrack
$$
We can define an \textbf{Order 2 Entering Pressure} $P^{E_2}$ by
$$
	\partial_x^2 P_i = \dfrac{P_{i-1} - 2P_{i,m}^E + P_\phi}{{h_i}^2} = \dfrac{P_{i-1}- 2P_i + P_{i+1}^{{E_2}}}{\delta_x^2}
$$
so that
$$
	P_{i+1}^{{E_2}} = 2P_i - P_{i-1} + \left(\dfrac{\delta_x}{h_i}\right)^2
	\left(
...
$$
	\sigma^L_i = \dfrac{P_{i+1}-P_\psi}{L_i^L}.
$$
...
$$
Using $h_i = \dfrac{L_i}{2}$, the pressure at the middle of the segment can be estimated
by
$$
...
defined by
$$
	P_{i-1}^{L_2} = 2P_i - P_{i+1} + \left(\dfrac{\delta_x}{h_i}\right)^2
	\left(
...
and a gradient
$$
	\sigma_i = \dfrac{P_\phi - P_\psi}{Li}
$$
so that setting
$$
	P^{L_1}_{i-1} = \dfrac{2\delta_x}{L_i} P_\psi
$$
and
$$
	P^{E_1}_{i+1} = \dfrac{2\delta_x}{L_i} P_\phi
$$
solves the order 1 estimation.

We use $h_i=\dfrac{L_i}{2}$.
...
$$
	P_{i,m} = \dfrac{1}{\phi}
	\left\lbrack
		h_i P_i + \left(\phi-h_i\right) P_\phi
	\right\rbrack
$$
\item If $\psi>\phi$, 
	$$
		P_{i,m} = \dfrac{1}{\psi}
	\left\lbrack
		h_i P_i + \left(\psi-h_i\right) P_\psi
	\right\rbrack
$$
...
$$
	\partial_x^2 P_i = \dfrac{P_\psi - 2P_{i,m} + P_\phi}{h_i^2} =
	\dfrac{P^{L_2}_{i-1} - 2P_i + P^{E_2}_{i+1}}{\delta_x^2}.
$$
We solve the order 2 problem by setting
$$
	P^{L_2}_{i-1} = P^{E_2}_{i+1} = P_i + \dfrac{1}{2}\left(\dfrac{\delta_x}{h_i}\right)^2 \left( P_\psi - 2P_{i,m} + P_\phi \right)
$$
