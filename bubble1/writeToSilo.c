...
void writeDomain2D(int count)
{
 ...
 //   int ny=1;
    ...
    /* Write a node-centered scalar variable */
    for(i=0;i<(indx_t) NC_s;i++)
    {
        DBPutQuadvar1(dbfile, cpntName_s[i], "quadmesh", &fields[i][zmin][0][xmin], dims,ndims, NULL, 0, DB_DOUBLE, DB_NODECENT, NULL);
    }
    /* Write a node-centered vector variable */

    for(i=0;i<(indx_t) NC_v;i++)
    {
        const char *varnames[] = {"velocity_x","velocity_y"};

        double *comp[2];
        comp[0]=&fields[NC_s+2*i][zmin][0][xmin];
        comp[1]=&fields[NC_s+2*i+1][zmin][0][xmin];
        DBPutQuadvar(dbfile,cpntName_v[i], "quadmesh",
                     2, (char **)varnames, comp, dims,
...
write_multivar(DBfile *dbfile,int cycle)
{
    ...
    {        
        ...
        for(dom = 0; dom < size; ++dom)
        {
            ...
            sprintf(tmp, "bubble%d.%d:%s", dom,cycle,cpntName_s[i]);
            varnames[dom] = strdup(tmp);
        }
        // Write the multivar. 
        
        DBPutMultivar(dbfile, cpntName_s[i], size, varnames, vartypes, NULL);
    ...
    {        
        ...
        for(dom = 0; dom < size; ++dom)
        {
            ...
            sprintf(tmp, "bubble%d.%d:%s", dom,cycle,cpntName_v[i]);
            varnames[dom] = strdup(tmp);
        }
        // Write the multivar. 
        
        DBPutMultivar(dbfile, cpntName_v[i], size, varnames, vartypes, NULL);
...
void write_curves(DBfile *dbfile,int cycle)
{ 
    int i, ndims = 2;
    char name[256]={"bubble"};

    DBPutCurve (dbfile,name ,b0.x,b0.z, DB_DOUBLE,NB,NULL);
}
void
