...
//    fprintf(stderr,"proc %d: xmin=%d\t xmax=%d\n",rank,(int)xmin,(int)xmax);
    ...
    for(i=xmin+1;i<=xmax-1;i++)
    {
        ...
        for(k=zmin;k<=zmax;k++)  
        {
            ...
            if((i+k)%2==color)
            {
                //resid=-0.25*(p[kp][j][i]+p[km][j][i]+p[k][j][im]+p[k][j][ip]-4.0*p[k][j][i]);
...
void computeVelocity()
{
    ...
    for(k=zmin;k<=zmax;k++)  
    {
        ...
        for(i=xmin;i<=xmax;i++)
        {
            ...
            if(i==Nx-1)
                ip=i;
            u[k][j][i]=-(p[k][j][ip]-p[k][j][im])/dx*0.5;
...
void advectBubble()
{
    ...
    for(i=0;i<NB;i++)
    {
        ...
        {
            ip=ceil(b0.x[i]/Lx*(Nx-1.0));
            kp=ceil(b0.z[i]/Lz*(Nz-1.0));
