...
%$$
$$
	\partial_x P_i = \dfrac{P_\phi - P_{i-1}}{\delta x + \phi} = \dfrac{P_i^R - P_{i-1}}{2\delta x}
$$
so that the \textit{Right Effective Pressure} or \textbf{Horizontal Entering Pressure} is set by
$$
	P_i^R = P_{i-1} + \dfrac{2\delta x}{\phi+\delta x} \left( P_\phi - P_{i-1} \right).
$$
\item
If the intersection is at $x_i-\psi$ with $P_\psi = P_{bubble} - \gamma \kappa_\psi$
then the pressure gradient can be estimated by
$$
	\partial_x P_i = \dfrac{P_{i+1} - P_\psi}{\delta x + \psi} = \dfrac{P_{i+1}-P_i^L}{2\delta x}
$$
with the \textit{Left Effective Pressure} or \textbf{Horizonal Leaving Pressure} set by
$$
	P_i^L = P_{i+1} - \dfrac{2\delta_x}{\delta x + \psi}\left(P_{i+1} - P_\psi\right)
$$
\item
If $x_i$ is surrounded by two intersections then
$$
	P_i^L = \dfrac{2\delta x}{\phi+\psi} P_\psi \text{ and } P_i^R = \dfrac{2\delta x}{\phi+\psi} P_\phi
$$
\end{itemize}
